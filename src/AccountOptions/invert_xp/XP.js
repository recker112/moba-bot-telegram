...
const xp_debuff = async (ctx) => {
	...
	if (!cancel && user) {
		...
		if (user_from.points >= 0) {
			...
			sql = `INSERT INTO debuffs(user_id, user_from, type, amount, xp_amount, expired_at) VALUES ($1, $2, 'xp_debuff', $3, $3 * 60, now()::timestamp + '8 hr'::INTERVAL)`;

			await client.query(sql, [user.id,ctx.from.id,params[1]]);
...
5) No puedes darte un debuff a ti mismo.

Si desea cancelar puede usar el comando /cancel.`);
	}else if (insert && !cancel_user) {
		response = await ctx.replyWithMarkdown(`@${ctx.from.username} pagó ${params[1] * 60}XP para darle un debufo a @${params[0]}`);
	} else {
