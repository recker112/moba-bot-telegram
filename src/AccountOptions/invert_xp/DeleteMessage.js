...
const delete_message_awaitResponse =  async (ctx) => {
	let response = await ctx.replyWithMarkdown(`Para agregar el debuffo de *ELIMINAR SIGUIENTE MENSAJE* use siguiente formato:
...
Para cancelar simplemente escriba /cancel.`);
	...
	if (session) {
		find = ctx.session.awaitResponse.findIndex(({type}) => type === 'delete_message');
	}else {
...
const delete_message = async (ctx) => {
	...
	if (!cancel && user) {
		...
		if (user_from.points >= 0) {
			...
			sql = `INSERT INTO debuffs(user_id, user_from, type, amount, xp_amount, expired_at) VALUES ($1, $2, 'delete_message', $3, $4, now()::timestamp + '48 hr'::INTERVAL)`;
	...
	response = await ctx.replyWithMarkdown(`No se ha podido agregar el debufo de *ELIMINAR SIGUIENTE MENSAJE* porque el *usuario* seleccionado *no se encuentra registrado*, por favor intente con uno registrado.

Si desea cancelar puede usar el comando /cancel.`);
	}else if (not_points && !insert && !cancel_user) {
		response = await ctx.replyWithMarkdown(`No se ha podido agregar el debufo de *ELIMINAR SIGUIENTE MENSAJE* porque *no dispone de ${not_points}XP* para realizar esta acción, por favor *intente con una cantidad menor*.

Si desea cancelar puede usar el comando /cancel.`);
	}else if (!insert && !cancel_user) {
		response = await ctx.replyWithMarkdown(`No se ha podido agregar el debufo de *ELIMINAR SIGUIENTE MENSAJE*, estas pueden ser sus causas:
