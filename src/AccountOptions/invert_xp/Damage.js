...
const damage_debuff_awaitResponse =  async (ctx) => {
	let response = await ctx.replyWithMarkdown(`Para agregar el debuffo de *DAÑO* use siguiente formato:
...
Para cancelar simplemente escriba /cancel.`);
	...
	if (session) {
		find = ctx.session.awaitResponse.findIndex(({type}) => type === 'damage_debuff');
	}else {
...
const damage_debuff = async (ctx) => {
	...
	if (!user && !insert && !cancel_user && !cancel) {
		response = await ctx.replyWithMarkdown(`No se ha podido agregar el debufo de *DAÑO* porque el *usuario* seleccionado *no se encuentra registrado*, por favor intente con uno registrado.

Si desea cancelar puede usar el comando /cancel.`);
	}else if (not_points && !insert && !cancel_user) {
		response = await ctx.replyWithMarkdown(`No se ha podido agregar el debufo de *DAÑO* porque *no dispone de ${not_points}XP* para realizar esta acción, por favor *intente con una cantidad menor*.

Si desea cancelar puede usar el comando /cancel.`);
	}else if (!insert && !cancel_user) {
		response = await ctx.reply(`No se ha podido agregar el debufo de DAÑO, estas pueden ser sus causas:
