...
const vida_base_awaitResponse =  async (ctx) => {
	...
	let sql = 'SELECT vida_base FROM configs WHERE id=1';
	...
	let response = await ctx.replyWithMarkdown(`La *VIDA BASE* actualmente se encuentra en *${configs.vida_base}*, para cambiarlo simplemente escriba la nueva cantidad.
...
Si desea cancelar esta acción puede usar /cancel.`);
	...
	if (session) {
		find = ctx.session.awaitResponse.findIndex(({type}) => type === 'vida_base');
	}else {
...
const vida_base = async (ctx) => {
	...
	// NOTA(RECEKR): Cambiar vida_base
	if (number && number > 0) {
		sql = 'UPDATE configs SET vida_base=$1 WHERE id=1';
	...
	if (!updated && !cancel_user) {
		response = await ctx.reply(`No se ha podido actualizar la vida base, estas pueden ser sus causas:
