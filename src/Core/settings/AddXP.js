...
const addxp_awaitResponse =  async (ctx) => {
	let response = await ctx.reply(`Para agregar xp a un usuario use el siguiente formato:
...
Para cancelar simplemente escriba /cancel`);
	...
	if (session) {
		find = ctx.session.awaitResponse.findIndex(({type}) => type === 'addxp');
	}else {
...
const addxp = async (ctx) => {
	...
	while (text[i]) {
		...
		if (!cancel && user) {
			sql = 'UPDATE experiences SET points=points+$1 WHERE user_id=$2';
	...
	if (!querys && !cancel_user) {
		response = await ctx.reply(`Para agregar xp a un usuario use el siguiente formato:
...
Si desea cancelar puede usar el comando /cancel.`);
		...
		response = await ctx.replyWithMarkdown('Experiencias agregadas!');
	} else {
