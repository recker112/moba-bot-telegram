...
Para cancelar simplemente escriba /cancel.`);
	...
	if (session) {
		find = ctx.session.awaitResponse.findIndex(({type}) => type === 'addword2');
	}else {
...
const addword2 = async (ctx) => {
	...
	if (!querys && !cancel_user) {
		response = await ctx.reply(`Para una *PALABRA CARIÑOSA* al sistema use el siguiente formato:
...
Si desea cancelar puede usar el comando /cancel.`);
		...
		response = await ctx.replyWithMarkdown('Palabras cariñosas agregadas!');
	} else {
